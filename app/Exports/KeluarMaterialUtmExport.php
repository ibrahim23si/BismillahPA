...
class KeluarMaterialUtmExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithEvents, ShouldAutoSize
{
    ...
    {
        return KeluarMaterialUtm::whereBetween('tanggal', [$this->startDate, $this->endDate])
            ->orderBy('tanggal', 'asc')->orderBy('nomor_urut', 'asc')->get();
    }

    public function headings(): array
    {
        return ['No', 'Tanggal', 'Hari', 'No. Tiket', 'Nopol', 'Transporter', 'Customer', 'Nama Barang', 'Gross (Ton)', 'Tara (Ton)', 'Netto (Ton)', 'Keterangan'];
    }

    public function map($row): array
    {
        $this->rowNumber++;
        return [$this->rowNumber, $row->tanggal->format('d/m/Y'), $row->hari, $row->nomor_tiket, $row->nopol, $row->transporter, $row->nama_customer, $row->nama_barang, $row->gross, $row->tara, $row->netto, $row->keterangan ?? ''];
    }
    ...
    public function title(): string { return 'Keluar Material UTM'; }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $lastRow = $sheet->getHighestRow(); $lastCol = 'L';
                ...
                $sheet->setCellValue('A1', 'DATA KELUAR MATERIAL UTM'); $sheet->mergeCells("A1:{$lastCol}1");
