...
class TerimaRawExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithEvents, ShouldAutoSize
{
    ...
    {
        return TerimaRaw::whereBetween('tanggal', [$this->startDate, $this->endDate])
            ->orderBy('tanggal', 'asc')->orderBy('nomor_urut', 'asc')->get();
    }

    public function headings(): array
    {
        return ['No', 'Tanggal', 'Hari', 'No. Tiket', 'Nopol', 'Transporter', 'Nama Supplier', 'Nama Barang', 'Gross (Ton)', 'Tara (Ton)', 'Netto (Ton)'];
    }

    public function map($row): array
    {
        $this->rowNumber++;
        return [$this->rowNumber, $row->tanggal->format('d/m/Y'), $row->hari, $row->nomor_tiket, $row->nopol, $row->transporter, $row->nama_supplier, $row->nama_barang, $row->gross, $row->tara, $row->netto];
    }
    ...
    public function title(): string { return 'Terima Raw'; }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $lastRow = $sheet->getHighestRow(); $lastCol = 'K';
                ...
                $sheet->setCellValue('A1', 'DATA TERIMA RAW'); $sheet->mergeCells("A1:{$lastCol}1");
