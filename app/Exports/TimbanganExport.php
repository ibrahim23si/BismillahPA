...
class TimbanganExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithEvents, ShouldAutoSize
{
    ...
    {
        return Timbangan::whereBetween('tanggal', [$this->startDate, $this->endDate])
            ->orderBy('tanggal', 'asc')->orderBy('nomor_urut', 'asc')->get();
    }

    public function headings(): array
    {
        return ['No', 'Tanggal', 'Hari', 'No. Tiket', 'Nopol', 'Transporter', 'Customer', 'Nama Barang', 'Gross (Ton)', 'Tara (Ton)', 'Netto (Ton)', 'Status', 'Harga Satuan', 'Total Harga', 'Keterangan'];
    }
    ...
    public function map($row): array
    {
        ...
        return [
            ...
            $row->tara,
            $row->netto,
            $row->status_jual ? 'Jual' : ($row->keterangan_lain ?: 'Lainnya'),
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                for ($row = 2; $row <= $lastRow; $row++) {
                    if ($row % 2 === 0) { $sheet->getStyle("A{$row}:{$lastCol}{$row}")->applyFromArray(['fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'F3F4F6']]]); }
                }

                $sheet->insertNewRowBefore(1, 3);
                $sheet->setCellValue('A1', 'DATA TIMBANGAN');
