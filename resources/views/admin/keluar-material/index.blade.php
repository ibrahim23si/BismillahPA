{{-- resources/views/admin/keluar-material/index.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        ...
        <script>
            $(document).ready(function() {
                $('#keluarMaterialTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('admin.keluar-material.index') }}",
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'DT_RowIndex',
                            orderable: false,
            ...
            function deleteData(id) {
                if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                    $.ajax({
                        url: "{{ route('admin.keluar-material.index') }}/" + id,
                        type: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                        success: function(response) {
                            if (response.success) {
                                $('#keluarMaterialTable').DataTable().ajax.reload();
