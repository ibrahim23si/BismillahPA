{{-- resources/views/admin/master/suppliers/index.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Data Master Supplier') }}
            </h2>
            <a href="{{ route('admin.master.suppliers.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                + Tambah Supplier
            </a>
        </div>
    </x-slot>
    ...
    <script>
        $(document).ready(function() {
            $('#supplierTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.master.suppliers.index') }}",
                columns: [
        ...
        function deleteData(id) {
            if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                $.ajax({
                    url: "{{ route('admin.master.suppliers.index') }}/" + id,
                    type: 'DELETE',
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
