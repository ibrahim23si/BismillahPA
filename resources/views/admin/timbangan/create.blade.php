{{-- resources/views/admin/timbangan/create.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Data Timbangan') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.timbangan.store') }}">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="nomor_urut" id="nomor_urut" value="{{ old('nomor_urut') }}"
                            ...
                            <div>
                                <label for="hari" class="block text-sm font-medium text-gray-700">Hari <span
                                        class="text-red-500">*</span></label>
                                <input type="number" name="hari" id="hari" value="{{ old('hari', date('d')) }}"
                                    min="1" max="31"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                    required>
                                @error('hari')
                            ...
                            <div>
                                ...
                                <select name="transporter" id="transporter"
                                    ...
                                    required>
                                    @if(old('transporter'))
                                        <option value="{{ old('transporter') }}" selected>{{ old('transporter') }}</option>
                                    @endif
                                </select>
                                @error('transporter')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="nama_customer" id="nama_customer"
                                    ...
                                    @if(old('nama_customer'))
                                        <option value="{{ old('nama_customer') }}" selected>{{ old('nama_customer') }}</option>
                                    @endif
                                </select>
                                @error('nama_customer')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="nama_barang" id="nama_barang"
                                    ...
                                    required>
                                    @if(old('nama_barang'))
                                        <option value="{{ old('nama_barang') }}" selected>{{ old('nama_barang') }}</option>
                                    @endif
                                </select>
                                @error('nama_barang')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
    ...
    @push('scripts')
        <script>
            ...
            initSelect2('#nama_customer', '{{ route("admin.master.customers.list") }}', 'Cari customer...');
