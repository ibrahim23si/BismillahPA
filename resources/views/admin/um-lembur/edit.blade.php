{{-- resources/views/admin/um-lembur/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Data UM & Lembur') }} - {{ $umLembur->nama }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.um-lembur.update', $umLembur->id) }}" id="umLemburForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <input type="month" name="periode" id="periode" value="{{ old('periode', $umLembur->periode->format('Y-m')) }}" 
                            ...
                            <div>
                                ...
                                <input type="text" name="nama" id="nama" value="{{ old('nama', $umLembur->nama) }}" 
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                    readonly>
                                @error('nama')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan', $umLembur->jabatan) }}" 
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                    readonly>
                                @error('jabatan')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" step="0.01" name="upah_per_jam" id="upah_per_jam" value="{{ old('upah_per_jam', $umLembur->upah_per_jam) }}" 
                            ...
                            <div class="md:col-span-3">
                                ...
                                <textarea name="keterangan" id="keterangan" rows="2" 
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('keterangan', $umLembur->keterangan) }}</textarea>
                                @error('keterangan')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr>
                                            @for($i = 1; $i <= 31; $i++)
                                                @php
                                                    $hari = 'hari_' . $i;
                                                    $nilai = old($hari, $umLembur->$hari);
                                                @endphp
                                                <td class="px-2 py-2 border">
                                                    <input type="number" step="0.5" min="0" max="24" name="hari_{{ $i }}" id="hari_{{ $i }}" value="{{ $nilai }}"
                        ...
                        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <span class="ml-2 text-lg font-bold text-blue-600" id="total-jam-text">{{ number_format($umLembur->total_jam, 2) }} jam</span>
                                </div>
                                <div>
                                    <span class="text-sm text-gray-600">Upah per Jam:</span>
                                    <span class="ml-2 text-lg font-bold text-green-600" id="upah-text">Rp {{ number_format($umLembur->upah_per_jam, 0, ',', '.') }}</span>
                                </div>
                                <div>
                                    <span class="text-sm text-gray-600">Total Upah:</span>
                                    <span class="ml-2 text-lg font-bold text-purple-600" id="total-upah-text">Rp {{ number_format($umLembur->total_upah, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
