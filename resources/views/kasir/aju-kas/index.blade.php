{{-- resources/views/kasir/aju-kas/index.blade.php --}}
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200" id="ajuKasTable">
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function() {
                $('#ajuKasTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('kasir.aju-kas.index') }}",
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'DT_RowIndex',
                            orderable: false,
            ...
            function deleteData(id) {
                if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                    $.ajax({
                        url: "{{ route('kasir.aju-kas.index') }}/" + id,
                        type: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                        success: function(response) {
                            if (response.success) {
                                $('#ajuKasTable').DataTable().ajax.reload();
