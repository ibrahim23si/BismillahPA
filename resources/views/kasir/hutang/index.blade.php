{{-- resources/views/kasir/hutang/index.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('kasir.hutang.create') }}"
                ...
                <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                    <div class="p-6">
                        ...
                        <p class="text-2xl font-semibold text-gray-900">Rp
                            {{ number_format($totalHutang ?? 0, 0, ',', '.') }}</p>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                    <div class="p-6">
                        <p class="text-sm font-medium text-gray-600">Hutang Jatuh Tempo</p>
                        <p class="text-2xl font-semibold text-red-600">Rp
                            {{ number_format($totalHutangJatuhTempo ?? 0, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        ...
        <script>
            ...
            function deleteData(id) {
                if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                    $.ajax({
                        url: "{{ route('kasir.hutang.index') }}/" + id,
                        type: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                        success: function(response) {
                            if (response.success) {
                                $('#hutangTable').DataTable().ajax.reload();
