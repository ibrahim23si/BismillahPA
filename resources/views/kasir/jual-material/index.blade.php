{{-- resources/views/kasir/jual-material/index.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        ...
        <script>
            $(document).ready(function() {
                $('#jualMaterialTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('kasir.jual-material.index') }}",
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'DT_RowIndex',
                            orderable: false,
            ...
            function deleteData(id) {
                if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                    $.ajax({
                        url: "{{ route('kasir.jual-material.index') }}/" + id,
                        type: 'DELETE',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
                        success: function(response) {
                            if (response.success) {
                                $('#jualMaterialTable').DataTable().ajax.reload();
